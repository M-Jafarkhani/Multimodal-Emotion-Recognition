...
class MultiplicativeInteractions3Modal(nn.Module):
    ...
    def __init__(self, input_dims, output_dim, task=None):
        ...
        self.b = MultiplicativeInteractions2Modal(
            [input_dims[0], input_dims[1]], output_dim, "matrix"
        )
        self.task = task
    ...
    def forward(self, modalities):
        ...
        if self.task == "affect":
            return torch.einsum(
                "bm, bmp -> bp", modalities[2], self.a(modalities[0:2])
            ) + self.b(modalities[0:2])
        return torch.matmul(modalities[2], self.a(modalities[0:2])) + self.b(
            modalities[0:2]
        )
...
class MultiplicativeInteractions2Modal(nn.Module):
    ...
    def __init__(
        ...
        if output == "matrix3D":
            ...
            self.U = nn.Parameter(
                torch.Tensor(input_dims[0], output_dim[0], output_dim[1])
            )
            nn.init.xavier_normal(self.U)
            self.V = nn.Parameter(
                torch.Tensor(input_dims[1], output_dim[0], output_dim[1])
            )
        ...
        # Diagonal Forms and Gating Mechanisms.
            ...
            nn.init.xavier_normal(self.W)
            self.U = nn.Parameter(
                torch.Tensor(self.input_dims[0], self.input_dims[1])
            )
            nn.init.xavier_normal(self.U)
        ...
        if grad_clip is not None:
            for p in self.parameters():
                p.register_hook(
                    lambda grad: torch.clamp(grad, grad_clip[0], grad_clip[1])
                )
    ...
    def forward(self, modalities):
        ...
        # Scales and Biases.
            ...
            output = self._repeatHorizontally(
                Wprime, self.input_dims[1]
            ) * m2 + self._repeatHorizontally(bprime, self.input_dims[1])
        return output
