...
class LeNet(nn.Module):
    ...
    def __init__(
        ...
        self.convs = [
            nn.Conv2d(
                in_channels, args_channels, kernel_size=5, padding=2, bias=False
            )
        ]
...
class VGG(nn.Module):
    """Extends models.vgg19 module with Global Pooling, BatchNorm, and a Linear Output."""
    ...
    def __init__(self, num_outputs):
        ...
        # self.vgg = models.vgg19(pretrained='imagenet')
        vgg = list(models.vgg19(pretrained="imagenet").features)
...
class DAN(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.has_padding:
            ...
            ).repeat(embedded.shape[0], 1) < x_lens.repeat(-1, 1).repeat(
                1, embedded.shape[1]
            )
            embedded[~mask] = 0
        # sum pooling
